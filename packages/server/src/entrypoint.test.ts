...
// server's `cli.ts` previously had NO guard at all, so its `sdc-server` dev script
...
describe("isDirectRun", () => {
  it("is true when argv[1] names this module's own path (a direct `tsx src/cli.ts` run)", () => {
    const p = "/repo/packages/server/src/cli.ts";
  ...
  it("is false when argv[1] is a different path (imported — the bin/*.mjs wrapper case)", () => {
    const moduleUrl = pathToFileURL("/repo/packages/server/src/cli.ts").href;
    const wrapper = "/repo/packages/server/bin/sdc-server.mjs";
