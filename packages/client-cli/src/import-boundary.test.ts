// SPEC §5.4 / §6.5, AGENTS.md §2 (INV-3): the reference client sees only
...
// client-cli source, so INV-3 is mechanically enforced rather than a convention.
...
// run with that as its cwd for the client-cli block to match.
...
describe("INV-3 import-boundary lint rule (client-cli)", () => {
  it("flags a client-cli file that imports the engine packages", async () => {
    const fixture = fileURLToPath(
      new URL("../test/fixtures/inv3-violation.ts", import.meta.url),
    );
    const code = readFileSync(fixture, "utf8");

    // Lint the fixture's code as if it lived in `client-cli/src`, so the
    // client-cli boundary block applies.
  ...
  it("passes the real client-cli source (imports only schema)", async () => {
    const results = await eslint().lintFiles([
      "packages/client-cli/src/**/*.ts",
    ]);
